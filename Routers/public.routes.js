 ...
 * Public, customer-facing shop profile — what a QR code scan resolves to.
 * Returns only public-safe fields (no phone/email unless the shop chose to
 * show them, no internal ids beyond the one needed to favourite the shop).
 */
router.get("/shop/:slug", async (req, res) => {
  try {
    ...
    const business = await BusinessProfileModel.findOne({ slug })
      .select(
        "businessName businessLogo bannerImage city businessAddress businessMapLocation perDayOpenHours specialDayOpenHours isBusinessActive ratings slug"
      )
      .lean();
    ...
    res.json({
      ...
      city: business.city,
      address: business.businessAddress,
      mapLocation: business.businessMapLocation || null,
      perDayOpenHours: business.perDayOpenHours || [],
      specialDayOpenHours: business.specialDayOpenHours || [],
      isBusinessActive: !!business.isBusinessActive,
      rating: avgRating,
      ratingCount: ratings.length,
    });
  } catch (err) {
    console.error("public/shop/:slug error:", err);
    res.status(500).json({ error: "Server error" });
  }
});
