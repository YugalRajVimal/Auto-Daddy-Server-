...
const AdminInvoiceSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    // NOTE: fixed the typo "FcPaid" -> "Paid" so this lines up with the frontend.
    // Added "PartiallyPaid" for the partial-payments flow.
    enum: ['Draft', 'Sent', 'PartiallyPaid', 'Paid', 'Overdue'],
    default: 'Draft',
  },
  // ── Partial payments ────────────────────────────────────────────────────
  // Running total of everything recorded via POST /:id/payments. Drives
  // `status` (Draft/Sent -> PartiallyPaid -> Paid) and, once > 0, locks the
  // invoice against edit/archive/delete (see controller).
      ...
      new mongoose.Schema(
        {
          ...
          method: { type: String, default: "" }, // e.g. "Cash", "Bank Transfer", "Cheque"
          note: { type: String, default: "" },
          paidAt: { type: Date, default: Date.now },
          recordedBy: { type: String, default: "" }, // staff name/email snapshot
        },
        { _id: true }
      ),
    ],
