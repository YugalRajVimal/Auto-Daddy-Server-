...
const jwtAuth = async (req, res, next) => {
  ...
  if (payload.role && STAFF_ROLES.includes(payload.role)) {
    try {
      ...
      // Single-session enforcement: a real login (adminVerifyAccount) always
      // overwrites activeSessionToken, so any older/other-device token stops
      // matching immediately. Impersonation tokens (loginAsStaffUser) are
      // exempt — they never touch activeSessionToken in the first place.
      if (!payload.isImpersonation && staff.activeSessionToken && staff.activeSessionToken !== token) {
        console.log(`[jwtAuth] Staff id=${payload.id} token superseded by a newer login elsewhere`);
        return res.status(401).json({
  ...
  try {
    ...
    // Single-session enforcement (see staff branch above for the same logic).
    if (!payload.isImpersonation && dbUser.activeSessionToken && dbUser.activeSessionToken !== token) {
      console.log(`[jwtAuth] User id=${dbUser._id} token superseded by a newer login elsewhere`);
      return res.status(401).json({
        message: "You have been logged out because this account was signed in on another device.",
      });
    }
