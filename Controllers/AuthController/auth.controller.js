...
import OtpDeviceThrottle from "../../Schema/OtpDeviceThrottle.schema.js";
...
// ── OTP generation throttle, per device ─────────────────────────────────────
// More than OTP_DEVICE_MAX_ATTEMPTS OTP requests from the same deviceId
// within OTP_DEVICE_WINDOW_MS blocks that device from requesting/verifying
// OTPs for OTP_DEVICE_BLOCK_MS. Deliberately keyed on deviceId (not on the
// User doc), so spamming OTPs across several phone numbers from one device
// still gets caught.
const OTP_DEVICE_WINDOW_MS = 30 * 60 * 1000; // rolling window: 30 minutes
const OTP_DEVICE_MAX_ATTEMPTS = 3; // 3 OTP generations allowed per window
const OTP_DEVICE_BLOCK_MS = 30 * 60 * 1000; // cooldown once exceeded: 30 minutes
...
/**
 * Call this BEFORE generating/sending a new OTP. Returns
 * { blocked: false } and bumps the attempt counter on success, or
 * { blocked: true, retryAfterMs } if the device should be refused.
 * `deviceId` is required to throttle — callers without one (very old
 * clients) are let through, since there's nothing to key the block on.
 */
async function checkAndRegisterOtpDeviceAttempt(deviceId, fcmToken) {
  if (!deviceId || typeof deviceId !== "string" || !deviceId.trim()) {
    return { blocked: false };
  }
  const now = new Date();
  let record = await OtpDeviceThrottle.findOne({ deviceId });

  if (record?.blockedUntil && record.blockedUntil > now) {
    return { blocked: true, retryAfterMs: record.blockedUntil.getTime() - now.getTime() };
  }

  if (!record) {
    await OtpDeviceThrottle.create({
      deviceId,
  ...
  if (record.attempts > OTP_DEVICE_MAX_ATTEMPTS) {
    record.blockedUntil = new Date(now.getTime() + OTP_DEVICE_BLOCK_MS);
    await record.save();
    return { blocked: true, retryAfterMs: OTP_DEVICE_BLOCK_MS };
  }
...
/** True if this device is currently mid-block (does NOT consume an attempt — use before verifyAccount too). */
async function isOtpDeviceBlocked(deviceId) {
  if (!deviceId || typeof deviceId !== "string" || !deviceId.trim()) return { blocked: false };
  const record = await OtpDeviceThrottle.findOne({ deviceId });
  const now = new Date();
  if (record?.blockedUntil && record.blockedUntil > now) {
    return { blocked: true, retryAfterMs: record.blockedUntil.getTime() - now.getTime() };
  }
...
class AuthController {
  ...
  signupAndLogin = async (req, res) => {
    try {
      let { countryCode, phone, email, deviceId, fcmToken } = req.body;
      ...
      // ── Per-device OTP throttle: block after 3 OTP requests from the
      //    same device within the rolling window (see helper above). ────────
      const throttle = await checkAndRegisterOtpDeviceAttempt(deviceId, fcmToken);
      if (throttle.blocked) {
        return res.status(429).json({
          message: `Too many OTP requests from this device. Please try again in ${formatBlockDuration(throttle.retryAfterMs)}.`,
        });
      }
      ...
      // Record the requesting device on the User doc too — previously this
      // only happened on successful verify, which meant a blocked device's
      // deviceId was never linked to the account it was hammering.
      if (deviceId) updateObj.deviceId = deviceId;
      if (typeof fcmToken === "string" && fcmToken.trim().length > 0) updateObj.fcmToken = fcmToken.trim();
  ...
  /**
   * Manual Auto Shop Owner signup (self-service — no Admin required).
   * POST /api/auth/autoshopowner/signup
   * Body: { countryCode?, phone, deviceId?, fcmToken? }
   *
   * - Creates a new "autoshopowner" User (phone/countryCode only) and sends
   *   a real OTP if one doesn't already exist for this phone.
   * - If the phone is already registered as an autoshopowner, this just
   *   (re)sends an OTP — safe to call again if signup was interrupted, and
   *   doubles as a "resend OTP" for manual login.
   * - If the phone is already registered under a different role, rejects.
   *
   * Next steps for the UI: POST /api/auth/verify-otp to get a JWT, then
   * PUT /api/autoshopowner/profile/complete-signup to fill in
   * name/businessName/businessLogo/city.
   */
  autoShopOwnerSignup = async (req, res) => {
    try {
      let { countryCode, phone, deviceId, fcmToken } = req.body;

      if (!phone || !String(phone).trim()) {
        return res.status(400).json({ message: "Phone number is required." });
      }

      countryCode = countryCode && countryCode.trim() ? countryCode.trim() : "+91";
      ...
      // ── Per-device OTP throttle (same rule as the rest of the OTP flows) ──
      const throttle = await checkAndRegisterOtpDeviceAttempt(deviceId, fcmToken);
      if (throttle.blocked) {
        return res.status(429).json({
          message: `Too many OTP requests from this device. Please try again in ${formatBlockDuration(throttle.retryAfterMs)}.`,
      ...
      if (user && user.role && user.role !== "autoshopowner") {
        return res.status(409).json({
          message: "This phone number is already registered with a different account type.",
        });
      }
      ...
      if (user) {
        // Already signed up (e.g. an interrupted signup, or a returning
        // owner) — just (re)send OTP, don't touch anything else.
        const updateObj = { otp, otpExpiresAt, otpGeneratedAt, otpAttempts: 0 };
        if (deviceId) updateObj.deviceId = deviceId;
        if (typeof fcmToken === "string" && fcmToken.trim().length > 0) updateObj.fcmToken = fcmToken.trim();
        await User.updateOne({ _id: user._id }, { $set: updateObj });
      } else {
        user = await User.create({
          ...
          status: "active",
          isDisabled: false,
          // Self-service signup — no Admin approval needed for this flow.
          onboardingStatus: "approved",
          onboardingRespondedAt: new Date(),
          ...(deviceId ? { deviceId } : {}),
          ...(typeof fcmToken === "string" && fcmToken.trim().length > 0 ? { fcmToken: fcmToken.trim() } : {}),
        });
      }

      // ── Send OTP SMS ────────────────────────────────────────────────────
      const smsMessage = isNewUser
        ? `Welcome to Auto Daddy! Your OTP to verify your Auto Shop signup is: ${otp}`
        : `Your OTP for Auto Daddy login is: ${otp}`;
      const normalizedTo = `${countryCode.replace('+', '')}${phone}`;
      const smsResult = await sendSms(normalizedTo, smsMessage);
      if (!smsResult.success) {
        console.error("[autoShopOwnerSignup] Error sending OTP SMS:", smsResult.error);
      }
      ...
      return res.status(isNewUser ? 201 : 200).json({
        ...
        message: isNewUser
          ...
          : "This phone number is already registered. OTP sent for login.",
        userId: user._id,
        isNewUser,
      ...
      console.error("[autoShopOwnerSignup] Error:", error);
      return res.status(500).json({ message: "Internal Server Error" });
    }
  };
  ...
  signUpLogInAndCompleteProfileAutoShopOwner = async (req, res) => {
    ...
    try {
      let { countryCode, phone, email, name, pincode, address, deviceId, fcmToken } = req.body;
      ...
      // ── Per-device OTP throttle (same rule as signupAndLogin) ────────────
      const throttle = await checkAndRegisterOtpDeviceAttempt(deviceId, fcmToken);
      if (throttle.blocked) {
        return res.status(429).json({
          message: `Too many OTP requests from this device. Please try again in ${formatBlockDuration(throttle.retryAfterMs)}.`,
      ...
      if (user) {
        ...
        if (typeof fcmToken === "string" && fcmToken.trim().length > 0) user.fcmToken = fcmToken.trim();
        if (!user.isProfileComplete) {
          user.name = name;
          user.email = email;
  ...
  verifyAccount = async (req, res) => {
    ...
    try {
      ...
      // Device-level cooldown from too many OTP requests also blocks
      // finishing login with whatever OTP is still outstanding.
      const throttle = await isOtpDeviceBlocked(deviceId);
      if (throttle.blocked) {
        return res.status(429).json({
          message: `Too many OTP requests from this device. Please try again in ${formatBlockDuration(throttle.retryAfterMs)}.`,
        });
      ...
      const updateObj = {
        ...
        phoneVerified: true,
        lastLogin: new Date(),
        // Single-session enforcement: this is now the ONLY valid token for
        // this account — jwtAuth rejects anything else, so any session on
        // another device is immediately signed out.
  ...
  adminSignin = async (req, res) => {
    try {
      ...
      // If phone is 9999999999, use "000000" as OTP and don't send SMS
      let otp;
      let skipSms = false;
      ...
      // Send OTP via SMS if phone is present and not 9999999999
      let smsResult;
      if (phone && !skipSms) {
      ...
      return res.status(200).json({ 
        ...
        otpSent: phone ? (skipSms ? false : !!smsResult?.success) : false
      });
    } catch (error) {
...
adminVerifyAccount = async (req, res) => {
  try {
    ...
    // Single-session enforcement: this is now the ONLY valid token for this
    // staff account — jwtAuth rejects anything else, so logging in here
    // immediately signs this account out of any other device/browser.
    staffUser.activeSessionToken = token;
  ...
  loginAsStaffUser = async (req, res) => {
    try {
      ...
      const tokenPayload = {
        ...
        email: staffUser.email,
        role: staffUser.role,
        // Exempts this token from single-session enforcement in jwtAuth —
        // impersonating a staff account for support purposes shouldn't sign
        // that staff member out of their own real session.
        isImpersonation: true,
      };
