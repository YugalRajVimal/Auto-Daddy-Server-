...
/**
 * Staff users are no longer created with a password chosen by the
 * SuperAdmin — the create form doesn't collect one. The schema still
 * requires `password` (a staff user must always have a valid hash), so we
 * generate a random, never-surfaced one here. The SuperAdmin then uses the
 * "Reset Password" action to set the real password before handing off
 * credentials to the new staff member.
 */
...
class StaffUserController {
  ...
  /**
   * POST /api/admin/staff-users  Body: { name, email, phone, roleId }
   *
   * No password is accepted from the client. A random temporary password
   * is generated so the schema's required `password` is always satisfied;
   * the SuperAdmin sets the real password afterwards via
  ...
  async create(req, res) {
    try {
      ...
      const staff = new StaffUser({
        ...
        email: email.toLowerCase(),
        phone: phone || "",
        password: generateTempPassword(), // never surfaced; SuperAdmin resets via dedicated endpoint
        role: roleDoc.type,   // role "type" (role_admin/sub_admin/associates) denormalized for auth
        roleRef: roleDoc._id, // source of truth for permissions
        createdBy: req.user.id,
      });
  ...
  /** PUT /api/admin/staff-users/:id — allows reassigning roleId (same type only). No password here. */
  async update(req, res) {
    try {
      const { name, email, phone, roleId } = req.body;
      const staff = await StaffUser.findById(req.params.id);
