...
// Human-readable labels for the staff roles that can perform an onboarding.
// "admin" is the SuperAdmin (StaffUser role "admin") and gets the special
// "Auto Daddy is onboarding you" copy; everyone else is addressed by name + role.
...
// How long the SMS approval link stays valid for.
const ONBOARDING_TOKEN_TTL_MS = 7 * 24 * 60 * 60 * 1000; // 7 days

/**
 * Builds the SMS body + generates the one-time approval token/expiry for a
 * newly-onboarded shop owner, based on who performed the onboarding
 * (req.user, populated by jwtAuth: { id, role, name, ... }).
 */
function buildOnboardingApproval(req, ownerName) {
  const token = crypto.randomBytes(32).toString("hex");
  const expiresAt = new Date(Date.now() + ONBOARDING_TOKEN_TTL_MS);

  const baseUrl = (process.env.ADMIN_ONBOARDING_APPROVAL_BASE_URL || "").replace(/\/$/, "");
  const approvalUrl = `${baseUrl}/onboarding-approval/${token}`;
  ...
  const message =
    staffRole === "admin"
      ? `Auto Daddy is onboarding you, ${ownerName}, as a shop owner. Please approve or reject here: ${approvalUrl}`
      : `${staffName} (${roleLabel}) is onboarding you, ${ownerName}, as a shop owner. Please approve or reject here: ${approvalUrl}`;
  ...
  return {
    ...
    message,
    approvalUrl,
    onboardedByStaff: req.user?.id || null,
    onboardedByStaffName: staffName || null,
    onboardedByStaffRole: staffRole || null,
  };
}
...
class AutoShopOwnerController {
      ...
      try {
        ...
        const autoShopOwnersRaw = await User.find(
          { role: "autoshopowner" },
          {
            ...
            city:1,
            onboardingStatus: 1,
            onboardedByStaffName: 1,
            onboardedByStaffRole: 1,
            onboardingRespondedAt: 1,
          }
        )
          .populate({
  ...
  createAutoShopOwner = async (req, res) => {
    try {
      ...
      // Only phone + shopType are mandatory now. name/email/pincode/address/city
      // are all optional — the shop owner fills these in themselves later
      // (see PUT /api/autoshopowner/profile/complete-signup).
      if (!phone || !shopType) {
        return res.status(400).json({
          success: false,
          message: "phone and shopType are required.",
        });
      }
      ...
      // ── Duplicate email check (only if an email was actually given) ──────
      ...
      // name/city/address/pincode/email are all optional now — the owner
      // fills these (and businessName/businessLogo) in themselves via
      // PUT /api/autoshopowner/profile/complete-signup.
      const businessProfileData = {
        businessName: name ? String(name).trim() : "",
        businessAddress: address ? String(address).trim().slice(0, 100) : "",
        city: city ? String(city).trim() : "",
        pincode: pincode ? String(pincode).trim() : "",
        businessPhone: String(phone).trim(),
        businessEmail: email ? String(email).trim().toLowerCase() : "",
      ...
      // ── Build the onboarding approval token/SMS copy for whoever is
      //    performing this onboarding (SuperAdmin vs named staff member) ────
      const onboarding = buildOnboardingApproval(req, name ? String(name).trim() : "there");
      ...
      const newOwner = await User.create({
        ...
        // Stays "pending" until the shop owner approves/rejects via the SMS link.
        onboardingStatus: "pending",
        onboardingApprovalToken: onboarding.token,
        onboardingTokenExpiresAt: onboarding.expiresAt,
        onboardedByStaff: onboarding.onboardedByStaff,
        onboardedByStaffName: onboarding.onboardedByStaffName,
        onboardedByStaffRole: onboarding.onboardedByStaffRole,
      });
      ...
      return res.status(201).json({
        success: true,
        message: "Auto shop owner created successfully. Awaiting their approval.",
        data: {
          _id: newOwner._id,
          name: newOwner.name,
          ...
          onboardingStatus: newOwner.onboardingStatus,
          onboardedByStaffName: newOwner.onboardedByStaffName,
          onboardedByStaffRole: newOwner.onboardedByStaffRole,
        },
      });
    } catch (err) {
