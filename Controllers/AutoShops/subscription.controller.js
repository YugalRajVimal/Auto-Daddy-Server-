...
//           data: {
//             currency: CURRENCY,
//             plans: plans.map((plan) => ({ ...plan, ...computeSubscriptionAmounts(plan) })),
//           },
//         });
//   } catch (error) {
...
//         success: false,
//         message: `Invalid planId. Only "yearly" is available.`,
//       });
//     }
...
import {
  handleWalletCheckoutCompleted,
  handleWalletCheckoutExpired,
  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        // This single webhook endpoint handles Checkout Sessions from more
        // than one flow — the Website subscription (below) and the
        // Software wallet recharge (wallet.controller.js) — distinguished
        // by session.metadata.type, since both use mode: "payment".
        if (session.metadata?.type === "wallet_recharge") {
          await handleWalletCheckoutCompleted(session);
          break;
        }
      ...
      case "checkout.session.expired": {
        ...
        if (session.metadata?.type === "wallet_recharge") {
          await handleWalletCheckoutExpired(session);
          break;
        }
