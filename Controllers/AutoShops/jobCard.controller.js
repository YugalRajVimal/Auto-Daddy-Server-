...
// import { assertCanCreateJobCard, chargeJobCardFee } from "./wallet.controller.js";
...
//     // --- NEW: check whether the shop owner has actually set a prefix
//     // for the current year, and whether a counter doc exists at all —
//     // independent of peekNextJobCardIdentifiers, which auto-creates/
//     // falls back so it can't tell the client "this hasn't been set up
//     // yet" on its own.
//     const currentYear = new Date().getFullYear();
//     const [prefixDoc, counterDoc, nextJobCard, myAllBanks] = await Promise.all([
//       getJobCardPrefixForYear(businessId, currentYear),
//       JobCardCounter.findOne({ business: businessId }),
//       peekNextJobCardIdentifiers(businessId),
//       AutoShopBank.find({ businessProfile: businessId }),
//     ]);

//     const isJobCardPrefixSet = !!prefixDoc;
//     const isJobCardCounterSet = !!counterDoc;
...
//     let walletGate;
//     try {
//       walletGate = await assertCanCreateJobCard(businessId);
//     } catch (gateErr) {
//       return res.status(gateErr.status || 402).json({
//         success: false,
//         code: gateErr.code,
...
//         // Deduct wallet fee (no-op during free trial)
//         try {
//           await chargeJobCardFee(businessId, jobCardNo, walletGate);
//         } catch (walletErr) {
//           console.error("[jobCard] wallet deduction failed after job card save:", walletErr);
//           // Job card already exists — don't fail the request over a billing
//           // bookkeeping error; flag it for reconciliation instead.
//         }
...
export const createJobCard = async (req, res) => {
  try {
    ...
    // Fast, advisory pre-check (see its doc comment in wallet.controller.js)
    // — lets us reject obviously-blocked requests with a 402 before doing
    // the heavier vehicle/customer/service validation below. This is NOT
    // the authoritative gate; `walletGate` itself is never charged or
    // trusted later in this function. The real, race-free check-and-deduct
    // is chargeJobCardFee(), called inside the transaction further down.
    try {
      await assertCanCreateJobCard(businessId);
    } catch (gateErr) {
      return res.status(gateErr.status || 402).json({
        success: false,
        code: gateErr.code,
    ...
    // Get next job card number and increment the counter (atomic update)
    // — NOTE: this now happens inside the transaction below, alongside the
    // wallet charge and the job card save, so a rollback of one rolls back
    // all three together.
    ...
    try {
      ...
      jobCardId = `${businessId}-${jobCardNo}`;
      ...
      // Atomic, race-free check-and-deduct — see chargeJobCardFee's doc
      // comment in wallet.controller.js. This MUST run in the same
      // transaction as the job card save: if the job card save below fails,
      // aborting the transaction rolls the deduction back too, and if the
      // balance is insufficient (including a balance another concurrent
      // request just spent), we abort before the job card is ever
      // persisted — no free/uncharged job cards, no orphaned charges.
      try {
        walletResult = await chargeJobCardFee(businessId, jobCardNo, session);
      } catch (walletErr) {
        await session.abortTransaction();
        return res.status(walletErr.status || 402).json({
          success: false,
          code: walletErr.code,
      ...
      // Covers anything unexpected between startTransaction() and
      // commitTransaction() that wasn't already caught (and aborted) above.
