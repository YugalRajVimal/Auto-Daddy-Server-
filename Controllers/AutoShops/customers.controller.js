...
// How long the SMS approval link stays valid for (same window as the Shop
// Owner onboarding-approval flow).
const CAR_OWNER_APPROVAL_TOKEN_TTL_MS = 7 * 24 * 60 * 60 * 1000; // 7 days
...
export const onboardCustomer = async (req, res) => {
  ...
  try {
    ...
    // Build the SMS approval token/link up front so it can be stored on the
    // myCustomers entry in the same push below.
    const approvalToken = crypto.randomBytes(32).toString("hex");
    const approvalTokenExpiresAt = new Date(Date.now() + CAR_OWNER_APPROVAL_TOKEN_TTL_MS);
    ...
    // ── Send onboarding approval-request SMS (Vonage) ────────────────────
    // Fire this after the transaction commits so a slow/failed SMS never
    // rolls back the customer record. Best-effort: failure is logged only.
    let smsSent = false;
    if (!alreadyInMyCustomers) {
      const baseUrl = (process.env.CAROWNER_APPROVAL_BASE_URL || "").replace(/\/$/, "");
      const approvalUrl = `${baseUrl}/car-owner-onboarding/${approvalToken}`;
      const shopName = business.businessName || "Your auto shop";
      const message = `${shopName} is onboarding you as a customer on Auto Daddy. Please approve or reject here: ${approvalUrl}`;
      const smsResult = await sendSms(
        `${usedCountryCode.replace("+", "")}${String(phone).trim()}`,
        message
      );
      smsSent = Boolean(smsResult.success);
      if (!smsResult.success) {
        console.error("[onboardCustomer] Error sending onboarding approval SMS:", smsResult.error);
      }
    }
...
/* =========================================================
   9. CAR OWNER ONBOARDING APPROVAL (public, no auth)
      The SMS sent from onboardCustomer() links here. The token
      itself is the credential — single-use (cleared once
      approved/rejected) and time-limited. Drives the SAME
      business.myCustomers[].status field the future in-app
      dashboard approval will use.

      NOTE: myCustomers.approvalToken is not globally unique by
      schema, so we scan across BusinessProfiles for a document
      whose myCustomers array contains a matching, still-pending
      token. In practice this is one indexed array field on one
      collection — fine at this scale; add a unique index on
      myCustomers.approvalToken if this needs to scale further.
   ========================================================= */

function findCarOwnerApprovalMessage(status) {
  if (status === "approved") return "You have already approved this onboarding request.";
  if (status === "rejected") return "You have already declined this onboarding request.";
  return null;
}

async function findBusinessByCarOwnerToken(token) {
  const business = await BusinessProfileModel.findOne({ "myCustomers.approvalToken": token });
  if (!business) return { business: null, entry: null };
  const entry = business.myCustomers.find((c) => c.approvalToken === token);
  return { business, entry: entry || null };
}

// GET /api/public/car-owner-onboarding/:token
export const getCarOwnerOnboardingByToken = async (req, res) => {
  try {
    const { token } = req.params;
    if (!token) return res.status(400).json({ success: false, message: "Token is required" });

    const { business, entry } = await findBusinessByCarOwnerToken(token);
    if (!entry) {
      return res.status(404).json({ success: false, message: "This onboarding link is invalid or has already been used." });
    }
    if (entry.approvalTokenExpiresAt && entry.approvalTokenExpiresAt < new Date()) {
      return res.status(410).json({ success: false, message: "This onboarding link has expired. Please ask the shop to resend it." });
    }

    return res.json({
      success: true,
      status: entry.status,
      alreadyResponded: entry.status !== "pending",
      infoMessage: findCarOwnerApprovalMessage(entry.status),
      customerName: entry.name || null,
      shopName: business.businessName || null,
      city: business.city || null,
      address: business.businessAddress || null,
    });
  } catch (error) {
    console.error("[getCarOwnerOnboardingByToken] error:", error);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};

// Shared handler for approve/reject so both stay in lock-step.
async function respondToCarOwnerOnboarding(req, res, nextStatus) {
  try {
    const { token } = req.params;
    if (!token) return res.status(400).json({ success: false, message: "Token is required" });
    ...
    if (entry.status !== "pending") {
      return res.status(409).json({
        success: false,
        message: findCarOwnerApprovalMessage(entry.status) || "This request has already been responded to.",
        status: entry.status,
      });
    }

    entry.status = nextStatus; // "approved" | "rejected" — rejecting leaves the User account active
    entry.respondedAt = new Date();
    // Single-use: clear the token so the link can't be replayed.
    entry.approvalToken = null;
    entry.approvalTokenExpiresAt = null;

    // On approval, apply any edits the shop owner made while onboarding
    // (kept in pendingEdit until now) to the customer's real User doc —
    // this is the behaviour already documented on myCustomerSchema.
    if (nextStatus === "approved" && entry.pendingEdit && (entry.pendingEdit.name || entry.pendingEdit.email || entry.pendingEdit.city)) {
      const update = {};
      if (entry.pendingEdit.name) update.name = entry.pendingEdit.name;
      if (entry.pendingEdit.email) update.email = entry.pendingEdit.email;
      if (entry.pendingEdit.city) update.city = entry.pendingEdit.city;
      await User.findByIdAndUpdate(entry._id, update);
      entry.pendingEdit = null;
    }

    await business.save();

    return res.json({ success: true, status: entry.status });
  } catch (error) {
    console.error(`[respondToCarOwnerOnboarding:${nextStatus}] error:`, error);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}

// POST /api/public/car-owner-onboarding/:token/approve
export const approveCarOwnerOnboarding = (req, res) => respondToCarOwnerOnboarding(req, res, "approved");

// POST /api/public/car-owner-onboarding/:token/reject
export const rejectCarOwnerOnboarding = (req, res) => respondToCarOwnerOnboarding(req, res, "rejected");