...
/**
 * Helper: delete a list of uploaded file paths (used for rollback on
 * validation failure, and for cleaning up replaced/removed images).
 * Safe to call with an empty/undefined array.
 */
 ...
 */
export const createDeal = async (req, res) => {
  let uploadedDealImages = [];
  try {
    console.log("Starting createDeal (services-rewrite)... Step 1: Fetching user.");
    const userId = req.user.id;
    const user = await User.findById(userId).lean();
    if (!user) {
      if (req.files?.length) await deleteUploadedFiles(req.files.map((f) => f.path));
      return res.status(404).json({ success: false, message: "User not found" });
    }
    ...
    if (req.files?.length) {
      // upload.array("dealImage", 2) already caps this at 2 at the multer
      // level (it throws a MulterError before this handler even runs if
      // more than 2 files are sent), but we double-check here too in case
      // route wiring ever changes.
      if (req.files.length > 2) {
        await deleteUploadedFiles(req.files.map((f) => f.path));
        return res.status(400).json({ success: false, message: "A deal can have at most 2 images." });
      }
      uploadedDealImages = req.files.map((f) => f.path);
    }
    ...
    let dealDoc = {
      dealType,
      createdBy: businessProfile._id,
      ...(uploadedDealImages.length && { dealImages: uploadedDealImages }),
    };
    ...
    if (dealType === "Service") {
      ...
      dealDoc = {
        ...
        discountPercentage,
        offerEndsOnDate: offerEndsDate,
        // For service deals, images are already handled under dealImages above
      };
 ...
 * deleted). If no files are sent, existing dealImages are left untouched.
 * There is no "append to existing" mode — the client should resend both
 * images if it only wants to change one.
 */
export const editDeal = async (req, res) => {
  let uploadedDealImages = [];
  let oldDealImages = [];
  try {
    ...
    let updates = {};
    oldDealImages = Array.isArray(deal.dealImages) ? deal.dealImages : [];
    ...
    if (req.files?.length) {
      ...
      uploadedDealImages = req.files.map((f) => f.path);
      updates.dealImages = uploadedDealImages;
    }
    ...
    // Only clean up old images once the update has actually succeeded, and
    // only the ones being replaced (skip any that happen to be reused).
    if (uploadedDealImages.length && oldDealImages.length) {
      const toDelete = oldDealImages.filter((p) => !uploadedDealImages.includes(p));
      if (toDelete.length) await deleteUploadedFiles(toDelete);
    }
...
export const deleteDeal = async (req, res) => {
  try {
    ...
    // Clean up the deal's image files on disk (both, if present).
    if (Array.isArray(deal.dealImages) && deal.dealImages.length) {
      await deleteUploadedFiles(deal.dealImages);
    }
...
export const fetchMyDeals = async (req, res) => {
  try {
    ...
    for (const deal of deals) {
      const createdAt = deal.createdAt ? deal.createdAt : null;
      const dealImages = Array.isArray(deal.dealImages) ? deal.dealImages : [];
